...
_Last regenerated: 2026-05-06 (kuramoto_continuum in GeneralGMainTheorem, exp 251)_

## Recently Updated

- [[continuum-stability-debate]] — GeneralGMainTheorem.lean: `kuramoto_continuum` (0 sorry, 0 axioms) — MINIMAL-HYPOTHESIS continuum theorem. Resolves all 3 reviewer problems (no uniform persistence, no bounded γ, no c_min). Body L² drop per M + tail vanishing (derived from probability measure). Also `kuramoto_continuum_of_global_gronwall` (bounded-γ special case). ALL g ∈ L¹(R).
