...
- [[continuum-stability-debate]] — KuramotoFirstMomentBarbalat (exp 310): CLOSES Student-t 1<ν≤2 with γ=|ω| gap. End-to-end for first-moment distributions without γ_min or second moment. Combines sc_fixed_point + new `kuramoto_standard_tendsto` in ContinuumSolvedFinal.lean (architectural fix: explicit r/α → `Tendsto r atTop` directly, bypassing Lean 4 theorem opacity). Drops hγ_sq_int + wired initialization vs EndToEndWired. Adds hα_neg + h_body_persist. Coverage: ALL ∫(1/γ)<∞, ∫γ<∞, γ>0 pointwise, body persistence. 0 sorry, 0 axioms. 2702 jobs.
- [[continuum-stability-debate]] — KuramotoFirstMomentEndToEndWired (exp 309): NON-VACUOUS end-to-end for Gaussian/Student-t ν>2. Drops hα_lb (vacuous for unbounded γ) + hμ_body_pos; adds hγ_sq_int + hα_bdd + hα_0_explicitEquil + r_min. Per-body δ₀(M) = explicitEquil(M,K,r₀) ~ Kr₀/(2M) → 0 correctly; C(M) ~ M²·τ(M) → 0 from second moment. Combines sc_fixed_point + explicit_init_wired7 + V7/V9 α_star derivations. 0 sorry, 0 axioms. 2715 jobs.
- [[continuum-stability-debate]] — KuramotoFirstMomentConcreteV9 + EndToEndV3 (exp 308): drops `hα_neg` (extension convention) via `hα_bdd : ∀ ω t, 0 ≤ α ω t ∧ α ω t ≤ 1` (global [0,1]-invariance). Key: for any t, hα_bdd + α*∈(0,1) → (α-α*)²≤1 without case split on sign of t. `nlinarith [ha.1, ha.2, hα_star_pos ω, hα_star_lt ω]` closes in one step. **hα_lb vacuousness**: `α₀_lb ≤ α ω t` for all t is inconsistent with convergence for unbounded γ. 0 sorry, 0 axioms. 2710 jobs.
- [[continuum-stability-debate]] — KuramotoFirstMomentEndToEndV2 (exp 307): cleanest end-to-end form. Combines V8 (no hα_sq_int) + sc_fixed_point (no explicit r_star). Signature: γ, K, hγ_pos, hγ_level, hγ_int, h_inv_int, hK_crit, ODE, hα_neg, hα_inv, persistence, hμ_body_pos → ∃ r_star∈(0,1), r(t)→r_star. No technical integrability beyond ODE structure. 0 sorry, 0 axioms. 2710 jobs.
- [[continuum-stability-debate]] — KuramotoFirstMomentConcreteV8 (exp 306): drops `hα_sq_int` from V7 by deriving it from `hα_neg + hα_inv`. For t≥0: α,α*∈(0,1) → (α-α*)²<1 via hα_inv. For t<0: hα_neg reduces to t=0 → same bound. `simp_rw [sq]; exact ha_int.1.mul ha_int.1` closes AEStronglyMeasurability. Lesson: use `by_cases ht : 0 ≤ t; push_neg at ht` (not `le_or_lt`). 0 sorry, 0 axioms.
- [[continuum-stability-debate]] — KuramotoFirstMomentEndToEnd (exp 305): end-to-end convergence without explicit r_star. Given K·∫(1/γ)dμ > 2 + ODE + persistence → ∃ r_star∈(0,1), r(t)→r_star. Key: hα_sq_int universally quantified over r_s so it can be specialized after fixed point extraction. 2-line proof: sc_fixed_point + v7. 0 sorry, 0 axioms.
- [[continuum-stability-debate]] — KuramotoContinuumSCFixedPoint (exp 304): `sc_fixed_point_exists_continuum` — K·∫(1/γ)dμ > 2 → ∃ r*∈(0,1) with r*=∫explicitEquil(γω,K,r*)dμ. Steps: rationalized form K·r/(γ+√(γ²+K²r²)) for DCT continuity; Φ(1)<1 via `integral_pos_iff_support_of_nonneg`; DCT slope limit ∫K/(2γ+Kr)→(K/2)∫1/γ>1; IVT via `isPreconnected_Icc.intermediate_value₂`. Key: `probReal_univ`; `nhdsWithin_le_nhds (s := Ioi 0)`. 0 sorry. 2521 jobs.
...
- [[continuum-stability-debate]] — KuramotoFirstMomentConcreteV5.lean (exp 301): `kuramoto_first_moment_concrete_v5` — drops hr_bdd by weakening BodyGronwallBound to accept ∀t>0. Derives ∀t>0, |r t|≤1 from r_abs_le_one_from_sc. 4 fewer hypotheses than exp 296. 0 sorry, 0 axioms.
...
- [[continuum-stability-debate]] — ContinuumSolvedWired5.lean (exp 279): `kuramoto_continuum_wired5` — eliminates `h_combined_vanish` from wired4. Derived from `hγ_sq_int : Integrable (γ²)` + `hδ₀_body_lb : ∃ c>0, c/M ≤ δ₀_body M`. Key: denominator ≥ C₂/M², so C(M) ≤ K/C₂·M²τ(M) → 0. Remaining open: `hμ_body_pos` only. 0 sorry, 0 axioms.
...
- [[continuum-stability-debate]] — FirstMomentBarbalat (exp 310): closes Student-t 1<ν≤2 γ=|ω| gap (first-moment, no γ_min, no second moment). Coverage: bounded γ → V9/EndToEndV3; Gaussian/St-t ν>2 → EndToEndWired; St-t 1<ν≤2 γ_min>0 → exp 295; Lorentzian → exp 287; St-t 1<ν≤2 γ=|ω| → FirstMomentBarbalat (exp 310). No remaining first-moment gaps.
