...
**first-moment-no-gmin-v5** — `kuramoto_first_moment_concrete_v5` (KuramotoFirstMomentConcreteV5.lean, 0 sorry, 0 axioms, exp 301): drops `hr_bdd` from V4. BodyGronwallBound.lean was weakened to only require `∀ t > 0, |r t| ≤ 1` (body Leibniz only uses s ∈ Ioi 0). V5 provides this via `r_abs_le_one_from_sc`. Net: 4 fewer hypotheses than exp 296 (hV_body_cont, hα_cont, hγ_meas, hr_bdd).

**first-moment-no-gmin-v6** — `kuramoto_first_moment_concrete_v6` (KuramotoFirstMomentConcreteV6.lean, 0 sorry, 0 axioms, exp 302): drops `hr_star_pos` from V5 via `integral_pos_iff_support_of_nonneg`: α* > 0 everywhere → support(α*)=univ → μ(univ)=1>0 → ∫α*>0=r*. Net: 5 fewer hypotheses than exp 296. Minimal form: ODE + equilibrium equation + first moment + level-set measurability + persistence + integrability.
...
**continuum-end-to-end-v2** — `kuramoto_first_moment_end_to_end_v2` (KuramotoFirstMomentEndToEndV2.lean, 0 sorry, 0 axioms, exp 307): cleanest end-to-end form. Combines V8 (no hα_sq_int) + sc_fixed_point (no explicit r_star). Signature: γ, K, hγ_pos, hγ_level, hγ_int, h_inv_int, hK_crit, ODE, hα_neg, hα_inv, persistence, hμ_body_pos → ∃ r_star∈(0,1), r(t)→r_star. No technical hypotheses beyond physical ODE structure.

**first-moment-v8** — `kuramoto_first_moment_concrete_v8` (KuramotoFirstMomentConcreteV8.lean, 0 sorry, 0 axioms, exp 306): drops `hα_sq_int` from V7, derives it from `hα_neg` + `hα_inv`. Key: for t≥0, α,α*∈(0,1) → (α-α*)^2<1; for t<0, use hα_neg to reduce to t=0 case. AEStronglyMeasurability: `simp_rw [sq]; exact ha_int.1.mul ha_int.1`. Lesson: `le_or_lt` not in scope; use `by_cases + push_neg` instead.
