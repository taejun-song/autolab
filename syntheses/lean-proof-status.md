...
Machine-checked proof status: 0 sorry, 0 axioms across 120 files. LorentzianExistence: complete ODE analysis — existence, uniqueness, convergence, rate, synchronization, parameter monotonicity, boundary behavior, Bernoulli linearization, fixed point, ODE sign analysis, semigroup property, trajectory sandwich, Lyapunov stability (δ=ε), strict Lyapunov function, V' ODE, exp Lyapunov bounds (below/above/unified), half-rate distance bounds, explicit convergence times (below + above), V positivity + zero characterization. 3336 build jobs.
