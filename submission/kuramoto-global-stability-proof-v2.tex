...
\item \textbf{Global stability without basin condition.} A DCT bootstrap (\texttt{r\_stays\_positive\_supercritical}, 0~sorry) proves $r(t) \geq r_{\min} > 0$ for $K > K_c$, removing the $V(0) < (r^*)^2$ assumption.
\item \textbf{Machine-checked formalization.} The full proof chain is verified in Lean~4 with 0~sorry, 0~axioms. Coverage includes Gaussian, Student-$t$ ($\nu > 1$), compact support, and mixtures whenever $\gamma(\omega) \lesssim 1 + |\omega|$ (ensuring $\int\gamma\,\mathrm{d}\mu < \infty$).
...
\begin{center}\small
\begin{tabular}{lllll}
\textbf{Model} & \textbf{Equation} & \textbf{Unconditional?} & \textbf{Lean status} & \textbf{Relation to Kuramoto} \\
\hline
Scalar dissipative & Eq.~\eqref{eq:oa} & Yes & 0 sorry, 0 axioms & Shares PLS self-consistency \\
...
Complex OA & Eq.~\eqref{eq:complex-oa} & No (1 axiom) & 0 sorry, 1 axiom & Standard OA reduction \\
...
\begin{theorem}[Global scalar stability, \texttt{r\_stays\_positive\_supercritical} + \texttt{hPsi\_floor\_of\_r\_liminf}]\label{thm:global}
...
For $n$-pole systems (any finite $n$), the theorem \texttt{kuramoto\_solved} gives \emph{global} stability: any $r(0) > 0$ converges. Persistence is automatic in finite dimensions. For the continuum scalar dissipative model~\eqref{eq:oa}, global stability is now also proved: \texttt{r\_stays\_positive\_supercritical} in \texttt{ContinuumInstability.lean} (0~sorry) shows that for $K > K_c$, the order parameter stays uniformly positive ($r(t) \geq r_{\min} > 0$) via a DCT bootstrap argument. The proof constructs $\varepsilon_0 > 0$ such that the self-consistency integral $\int\min(\alpha(\omega,0),\, K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0))\,\mathrm{d}\mu > \varepsilon_0$, then uses body persistence to show $r$ can never first cross below $\varepsilon_0$ (IVT + closed crossing set contradiction). This removes the basin condition $V(0) < (r^*)^2$ for the scalar dissipative model.
\end{remark}
...
\medskip
\noindent\textbf{Theorem A$'$: Real scalar global} (\texttt{r\_stays\_positive\_supercritical} + \texttt{hPsi\_floor\_of\_r\_liminf} + \texttt{real\_scalar\_complete}). Fully formalized, 0~sorry, 0~axioms. \textbf{Global} stability for the scalar dissipative model: for $K > K_c := 2/\int(1/\gamma)\,\mathrm{d}\mu$ and $\alpha(\omega,0) \in (0,1)$, the order parameter converges $r(t) \to r^*$ \emph{without} assuming $V(0) < (r^*)^2$. The DCT bootstrap (\texttt{ContinuumInstability.lean}) proves $r(t) \geq r_{\min} > 0$ uniformly, which then enters the basin machinery of Theorem~A.
...
\texttt{real\_scalar\_global} (Thm.~\ref{thm:global}) & Scalar dissipative & $\int\gamma + \int 1/\gamma < \infty$, $K > K_c$ & $r(t) \to r^*$ & 0 & 0 \\
