...
\textbf{Contribution 3: Formalization.} The Lean~4 formalization (Mathlib v4.30.0-rc1) spans \textbf{255+ Lean files}; the main theorems have \textbf{0~\texttt{sorry}} and \textbf{0 project axioms}. The basin stability theorem \texttt{real\_scalar\_complete} proves: for the scalar dissipative mean-field equation~\eqref{eq:oa} with $\gamma > 0$ pointwise, finite first moment $\int\gamma\,g < \infty$, and initial data in the basin $V(0) < (r^*)^2$, the order parameter converges $r(t) \to r^*$. The \textbf{global stability} extension (\texttt{r\_stays\_positive\_supercritical} + \texttt{hPsi\_floor\_of\_r\_liminf}) removes the basin condition entirely for $K > K_c$: a DCT bootstrap proves $r(t) \geq r_{\min} > 0$ uniformly, which then feeds into the basin machinery. The proof chain wires: supercritical DCT bootstrap ($r > 0$) $\to$ pair bound ($\dot V \leq 0$) $\to$ ODE barrier (body persistence) $\to$ Barbalat ($V \to 0$) $\to$ convergence ($r \to r^*$). When $\gamma(\omega) \lesssim 1 + |\omega|$, this covers Gaussian, compactly supported, Student-$t$ ($\nu > 1$), and mixtures. To our knowledge, this is the first fully self-contained, machine-checked global stability theorem for any Kuramoto-type model.
\end{abstract}
