...
\begin{abstract}
We introduce two novel proof techniques for Kuramoto model stability and present the first machine-checked formalization of any Kuramoto model result. The complex OA and full PDE results are conditional on two cited axioms (nonlinear Landau damping and OA manifold attractivity).

\textbf{Contribution 1: The pair bound.} A pointwise sum-of-squares inequality, lifted to the continuum double integral via Fubini, that proves the $L^2$ Lyapunov derivative $\dot V \leq 0$ for the scalar OA mean-field equation. This is a purely algebraic technique with no precedent in the Kuramoto literature, replacing spectral theory, Volterra equations, and Fourier analysis approaches.
...
\textbf{Contribution 3: Formalization.} The Lean~4 formalization (Mathlib v4.30.0-rc1) spans \textbf{255 Lean files} (commit \texttt{58b6194}) with \textbf{0~\texttt{sorry}}; \texttt{lake build KuramotoLean.RealScalarComplete} produces 0 sorry warnings. The main theorem \texttt{real\_scalar\_complete} (\textbf{0~sorry, 0~axioms, no hidden hypotheses}) proves: for the scalar dissipative mean-field equation~\eqref{eq:oa} with $\gamma > 0$ pointwise, finite first moment $\int\gamma\,g < \infty$, and initial data in the basin $V(0) < (r^*)^2$, the order parameter converges $r(t) \to r^*$. Body persistence is \emph{derived internally}, not assumed. The proof wires: pair bound ($\dot V \leq 0$) $\to$ Cauchy--Schwarz ($r \geq r^* - \sqrt{V(0)} > 0$) $\to$ ODE barrier (body persistence from $r > 0$) $\to$ Barbalat ($V \to 0$) $\to$ convergence ($r \to r^*$). Every link is machine-checked. Coverage: Gaussian, Student-$t$ with $\nu > 1$, compact support, and all mixtures. To our knowledge, this is the first fully self-contained, machine-checked stability theorem for any Kuramoto-type model.
\end{abstract}
...
\begin{proposition}[Derived monotonicity and convergence, \texttt{hV\_rate}]\label{prop:rate}
...
\begin{proof}
Decompose $V = V_{\mathrm{body}} + V_{\mathrm{tail}}$ where $V_{\mathrm{body}} = \int_{\gamma \leq M} (\alpha - \alpha^*)^2\,g\,\mathrm{d}\omega$ and $V_{\mathrm{tail}} = \int_{\gamma > M} (\alpha - \alpha^*)^2\,g\,\mathrm{d}\omega$. By choosing $M$ large, $V_{\mathrm{tail}} \leq \int_{\gamma > M} g\,\mathrm{d}\omega$ is small (since $\int \gamma\,g < \infty$ implies $g(\{\gamma > M\}) \to 0$). On the body, body persistence gives a coercive lower bound on $-V'$, so Barbalat's lemma (applied to $V$ which is non-negative, antitone, and has $V'$ uniformly continuous) gives $V_{\mathrm{body}}(t) \to 0$. This is formalized in \texttt{ContinuumBarbalat.lean}.
\end{proof}
...
\end{enumerate}
This is assembled as \texttt{kuramoto\_solved} in \texttt{GeneralGMainTheorem.lean}. The only external input beyond physical parameters is the persistence hypothesis~(v) of \texttt{h\_exists}; all intermediate estimates are derived.
\end{proof}
...
The formalization spans \textbf{255 Lean~4 files} (commit \texttt{58b6194}, Mathlib v4.30.0-rc1) with \textbf{0~\texttt{sorry}} and \textbf{0 additional axioms} beyond Lean~4's standard foundations, verified by \texttt{lake build KuramotoLean.RealScalarComplete} producing 0 sorry warnings.
...
\medskip
\noindent\textbf{Theorem A: Real scalar complete} (\texttt{real\_scalar\_complete}). Fully formalized, 0~sorry, 0~axioms. Basin stability for the scalar dissipative model~\eqref{eq:oa} with finite first moment $\int \gamma\,g < \infty$. Coverage: Gaussian, Student-$t$ ($\nu > 1$), compact support, all mixtures.
