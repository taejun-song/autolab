...
\textbf{Contribution 3: Formalization.} The Lean~4 formalization spans \textbf{235+ files} with \textbf{0~\texttt{sorry}}. The main theorem \texttt{real\_scalar\_complete} (\textbf{0~sorry, 0~axioms, no hidden hypotheses}) proves: for the scalar dissipative mean-field equation~\eqref{eq:oa} with $\gamma > 0$ pointwise, finite first moment $\int\gamma\,g < \infty$, and initial data in the basin $V(0) < (r^*)^2$, the order parameter converges $r(t) \to r^*$. Body persistence is \emph{derived internally}, not assumed. The proof wires: pair bound ($\dot V \leq 0$) $\to$ Cauchy--Schwarz ($r \geq r^* - \sqrt{V(0)} > 0$) $\to$ ODE barrier (body persistence from $r > 0$) $\to$ Barbalat ($V \to 0$) $\to$ convergence ($r \to r^*$). Every link is machine-checked. Coverage: Gaussian, Student-$t$ with $\nu > 1$, compact support, and all mixtures. This is the first fully self-contained, machine-checked stability theorem for any Kuramoto-type model.
\end{abstract}
