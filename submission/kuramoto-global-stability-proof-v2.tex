...
\textbf{Contribution 3: Formalization.} The Lean~4 formalization spans \textbf{235+ files} with \textbf{0~\texttt{sorry}}. Fully machine-checked results include: (a)~scalar model convergence $r(t) \to r^*$ for finite-first-moment distributions with body persistence (pair bound + Barbalat, 0~axioms); (b)~Dietert energy identity $\mathrm{d}\Psi/\mathrm{d}t = K|\eta|^2$; (c)~symmetry preservation via coupled uniqueness (no circularity); (d)~Gronwall bootstrap for basin-trapped decay. Applications to the standard complex OA are conditional on a perturbative pair bound in the basin (\texttt{kuramoto\_fresh}) or on cited published results (\texttt{kuramoto\_stability\_complex\_oa}, 2~axioms). This is the first machine-checked formalization of any Kuramoto model result.
\end{abstract}
