...
\]
where $\delta^*(M) = \min_{\gamma \leq M} \alpha^*(\omega) > 0$. Since $V$ is antitone and $V' \leq -c \cdot V_{\mathrm{body},M}$ with $c > 0$: the time-integral $\int_0^\infty V_{\mathrm{body},M}(t)\,\mathrm{d}t$ converges (bounded by $V(0)/c$). On $\{\gamma \leq M\}$, the vector field is uniformly Lipschitz and bounded (since $|\dot\alpha| \leq \gamma(\omega) + K \leq M + K$), hence $V_{\mathrm{body},M}$ is uniformly continuous. Since $V_{\mathrm{body},M} \geq 0$ and $\int_0^\infty V_{\mathrm{body},M}(t)\,\mathrm{d}t < \infty$, Barbalat's lemma gives $V_{\mathrm{body},M}(t) \to 0$.
