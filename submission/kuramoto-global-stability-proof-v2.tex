...
\begin{abstract}
We introduce two novel proof techniques for Kuramoto model stability and present the first machine-checked formalization of any Kuramoto model result.
...
\textbf{Contribution 3: Formalization.} The Lean~4 formalization spans \textbf{235+ files} with \textbf{0~\texttt{sorry}}. The end-to-end theorem \texttt{kuramoto\_complete} (\textbf{0~sorry, 0~axioms}) proves: for the standard complex OA equation with symmetric~$g$ having finite first moment, $K > 0$, initial data in the basin $V(0) < (r^*)^2$, and a perturbative coercivity condition (real pair bound dominates imaginary error), $\re(\eta(t))^2 \to (r^*)^2$. The proof wires: complex Leibniz (dominated convergence for $V'$) $\to$ rotation cancellation ($-i\omega$ drops out) $\to$ coercivity bound ($V' \leq -\mathrm{rate} \cdot V$) $\to$ Gronwall bootstrap ($V \to 0$) $\to$ Cauchy--Schwarz ($\re(\eta) \to r^*$). Every link is machine-checked. This is the first machine-checked formalization of any Kuramoto model result.
\end{abstract}
