...
Under hypotheses (i), (ii), (iv) of Theorem~\ref{thm:main}, assume additionally that $K > K_c := 2/\int(1/\gamma)\,\mathrm{d}\mu$ (supercritical coupling) and $\int(1/\gamma)\,\mathrm{d}\mu < \infty$. Then $r(t) \to r^*$ as $t \to +\infty$, \textbf{without} assuming the basin condition $V(0) < (r^*)^2$.
\end{theorem}

\begin{remark}
Theorem~\ref{thm:main} is a component of Theorem~\ref{thm:global}. The DCT bootstrap of Section~\ref{sec:bootstrap} proves $r(t) \geq r_{\min} > 0$ uniformly for all $t \geq 0$, which feeds into the basin machinery to discharge (iii). The scalar dissipative equation~\eqref{eq:oa} is \textbf{not} the standard complex OA equation~\eqref{eq:complex-oa}; it shares the PLS self-consistency relation but has different transient dynamics.
\end{remark}
...
Extract $N$ with $\int G_N\,\mathrm{d}\mu > 1$ and $1/(N{+}1) < r(0)$. Set $\varepsilon_0 := 1/(N{+}1)$. Then $\varepsilon_0 < r(0)$, $\varepsilon_0 \leq 1$, and
\[
\varepsilon_0 < \int \min\!\bigl(\alpha(\omega,0),\; K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0)\bigr)\,\mathrm{d}\mu.
\]
...
Proposition~\ref{prop:rpos} gives $r(t) \geq r_{\min} > 0$. The bridge theorem \texttt{hPsi\_floor\_of\_r\_liminf} feeds this into the body persistence $+$ Barbalat machinery of Theorem~\ref{thm:main} to obtain $V(t) \to 0$, hence $V(T_0) < (r^*)^2$ for some $T_0$. Theorem~\ref{thm:main} then gives $r(t) \to r^*$.
\end{proof}
...
\texttt{real\_scalar\_global} (Thm.~\ref{thm:global}) & Scalar dissipative & $\int\gamma\,g < \infty$, $K > K_c$ & $r(t) \to r^*$ & 0 & 0 \\
...
\begin{enumerate}
\item \textbf{Basin of attraction (resolved for scalar OA).} The continuum basin stability result (Theorem~A) requires $V(0) < (r^*)^2$. This condition is now \emph{discharged} for the scalar dissipative model: \texttt{r\_stays\_positive\_supercritical} in \texttt{ContinuumInstability.lean} (0~sorry, 0~axioms) proves that for $K > K_c$, the order parameter stays uniformly positive ($r(t) \geq r_{\min} > 0$ for all $t \geq 0$), via a DCT bootstrap and self-consistency contradiction. Combined with \texttt{hPsi\_floor\_of\_r\_liminf}, this gives global stability (Theorem~A$'$) without the basin condition. The restriction to the scalar dissipative model~\eqref{eq:oa} remains (not the standard complex OA).
