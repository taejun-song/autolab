...
\begin{abstract}
We prove global stability ($r(t) \to r^*$ for all initial data with $\alpha(\omega,0) \in (0,1)$) for a scalar dissipative mean-field equation sharing the self-consistency structure of the Kuramoto--Ott--Antonsen system. The scalar equation is not the OA equation; it shares the positive real PLS self-consistency relation, and hence the corresponding $K_c$, $\alpha^*$, and $r^*$ branch, but has different transient dynamics. The proof introduces a novel \emph{pair bound}---a pointwise algebraic inequality lifted to a continuum double integral via Fubini---that gives $\dot V \leq 0$ for the $L^2$ Lyapunov function without spectral theory or Fourier analysis. Global stability (no basin condition) follows from a dominated convergence bootstrap proving $r(t) \geq r_{\min} > 0$ uniformly for $K > K_c$. The entire proof chain is formalized in Lean~4 (Mathlib v4.30.0-rc1) with \textbf{0~\texttt{sorry}} and \textbf{0~project axioms}. A conditional extension to the standard complex OA equation and full Kuramoto PDE is also formalized (0~sorry, 2~cited axioms: nonlinear Landau damping and OA manifold attractivity). To the best of our knowledge, this is the first Lean formalization of a stability theorem for a Kuramoto-type model.
\end{abstract}

\paragraph{Main contributions.}
\begin{enumerate}[label=(\arabic*)]
\item \textbf{Pair bound.} A pointwise algebraic pair inequality (Proposition~\ref{prop:pair}) proving $\dot V \leq 0$ for the scalar dissipative model~\eqref{eq:oa}. No precedent in the Kuramoto literature.
\item \textbf{Global stability without basin condition.} A DCT bootstrap (0~sorry) proves $r(t) \geq r_{\min} > 0$ for all $K > K_c$, removing the basin assumption $V(0) < (r^*)^2$.
\item \textbf{Machine-checked formalization.} The full proof chain is verified in Lean~4 with 0~sorry, 0~axioms. Coverage: Gaussian, Student-$t$ ($\nu > 1$), compact support, and mixtures whenever $\gamma(\omega) \lesssim 1 + |\omega|$ ($\int\gamma\,\mathrm{d}\mu < \infty$).
\item \textbf{Conditional complex OA/PDE bridge.} Rotation cancellation in the Lyapunov derivative plus two external axioms gives $\|\eta(t)\| \to r^*$ for the standard complex OA and full Kuramoto PDE.
\end{enumerate}
...
For symmetric~$g$ with symmetric initial data ($z(\omega,0) = \overline{z(-\omega,0)}$), the symmetry is preserved and $\eta(t) = r(t) \in \R$ (Lean: \texttt{complex\_oa\_\allowbreak symmetry\_preserved}, 0~sorry). For the Lorentzian $g(\omega) = \gamma/(\pi(\omega^2\!+\!\gamma^2))$, residue evaluation at $\omega = -i\gamma$ gives~$\dot{r} = (K/2-\gamma)r - (K/2)r^3$.
...
\end{equation}
where $\alpha(\omega,t) \in (0,1)$ and $\gamma(\omega) > 0$ is a dissipation rate. Throughout, we write $\mathrm{d}\mu = g(\omega)\,\mathrm{d}\omega$ for the probability measure with density~$g$; integrals $\int f\,\mathrm{d}\mu$ and $\int f\,g\,\mathrm{d}\omega$ are used interchangeably. We assume $g \geq 0$, $\int g = 1$, and $g$ measurable.

This is a \emph{separate dynamical system} from~\eqref{eq:complex-oa}: the term $-\gamma(\omega)\alpha$ is dissipative, whereas $-i\omega z$ in~\eqref{eq:complex-oa} is rotational. The scalar model is designed to share the same positive real self-consistency equation as the symmetric partially locked state, but its transient dynamics differ from the standard complex OA equation.

\begin{remark}[Scope clarification]\label{rem:scope}
Equation~\eqref{eq:oa} is \textbf{not} derived from the complex OA equation~\eqref{eq:complex-oa} by setting $z = \alpha \in \R$. The subspace $\im(z) = 0$ is not invariant under~\eqref{eq:complex-oa} (since $\dot{y} = -\omega x \neq 0$ in general). The Lyapunov argument in Sections~\ref{sec:proof}--\ref{sec:cauchy} applies directly to~\eqref{eq:oa}. The conditional extension to the standard complex OA~\eqref{eq:complex-oa} is given in Section~\ref{sec:complex-extension}.
\end{remark}
...
\hline
Scalar dissipative & \eqref{eq:oa} & Yes & 0 sorry, 0 ax. & Shares PLS self-consistency \\
Lorentzian ODE & Bernoulli & Yes & 0 sorry, 0 ax. & Classical OA for Lorentzian $g$ \\
$n$-pole & Finite & Yes & 0 sorry, 0 ax. & Discrete analogue \\
Complex OA & \eqref{eq:complex-oa} & No (1 ax.) & 0 sorry, 1 ax. & Standard OA reduction \\
Full Kuramoto PDE & Kinetic & No (2 ax.) & 0 sorry, 2 ax. & Original model \\
\end{tabular}
\end{center}
...
\begin{theorem}[Basin stability, \texttt{real\_scalar\_complete}]\label{thm:main}
Let $\gamma(\omega) > 0$ with $\int \gamma\,\mathrm{d}\mu < \infty$ (finite first moment). Let $(\alpha^*(\omega), r^*)$ satisfy~\eqref{eq:equil} and~\eqref{eq:sc}. Let $(r, \alpha)$ solve~\eqref{eq:oa}:
\begin{enumerate}[label=(\roman*)]
\item $\alpha(\omega,\cdot)$ solves~\eqref{eq:oa} for all $\omega$, with $r(t) = \int \alpha(\omega,t)\,\mathrm{d}\mu$;
\item $\alpha(\omega,t) \in (0,1)$ for all $\omega, t \geq 0$ (invariant region);
\item $V(0) := \int (\alpha(\omega,0) - \alpha^*(\omega))^2\,\mathrm{d}\mu < (r^*)^2$ (initial closeness to PLS);
...
\begin{theorem}[Global scalar stability]\label{thm:global}
Let $\gamma(\omega) > 0$ with $\int \gamma\,\mathrm{d}\mu < \infty$ and $\int (1/\gamma)\,\mathrm{d}\mu < \infty$. Let $(\alpha^*, r^*)$ be the unique positive equilibrium satisfying~\eqref{eq:equil}--\eqref{eq:sc}. Let $(r, \alpha)$ solve~\eqref{eq:oa} with $r(t) = \int \alpha(\omega,t)\,\mathrm{d}\mu$, $\alpha(\omega,t) \in (0,1)$ for all $\omega, t \geq 0$, and initial body coherence (for each $M > 0$, $\exists\,\delta_0 > 0$ with $\alpha(\omega,0) \geq \delta_0$ on $\{\gamma \leq M\}$). Assume $K > K_c := 2/\!\int\!(1/\gamma)\,\mathrm{d}\mu$.

Then $r(t) \to r^*$ as $t \to +\infty$, \textbf{without} assuming the basin condition $V(0) < (r^*)^2$.
\end{theorem}

\begin{remark}
The condition $K > K_c$ does not by itself guarantee existence of $(\alpha^*, r^*)$; existence is an additional hypothesis (proved for specific families such as Gaussian and Student-$t$ via the implicit function theorem on the self-consistency equation). The scalar dissipative equation~\eqref{eq:oa} is \textbf{not} the standard complex OA equation~\eqref{eq:complex-oa}; it shares the PLS self-consistency relation but has different transient dynamics.
\end{remark}
...
\]
since $|\alpha - \alpha^*| \leq 1$ and $|\dot{\alpha}| \leq \gamma(\omega) + K$ for $\alpha, r \in (0,1)$. The dominator $2(\gamma + K)$ is integrable since $\int \gamma\,g\,\mathrm{d}\omega < \infty$ by hypothesis. Mathlib's differentiation-under-the-integral lemma then gives the stated formula for~$\frac{\mathrm{d}}{\mathrm{d}t}\!\int (\alpha - \alpha^*)^2\,g\,\mathrm{d}\omega$.
\end{proof}
...
\begin{proof}
Set $p_j = \alpha_j - \alpha^*_j$, $p_k = \alpha_k - \alpha^*_k$. Since $\alpha^*_j, \alpha^*_k > 0$, it suffices to show $\alpha^*_j \alpha^*_k \cdot (\text{LHS} - \text{RHS}) \geq 0$. Expanding the LHS terms and collecting:
\begin{align}
\alpha^*_j \alpha^*_k \cdot (\text{LHS} - \text{RHS}) &= \underbrace{(\alpha^*_j p_k - \alpha^*_k p_j)^2}_{T_1} + \underbrace{(\alpha^*_j)^2 \alpha_k \alpha^*_k\, p_k^2}_{T_2} \notag \\
&\quad + \underbrace{(\alpha^*_k)^2 \alpha_j \alpha^*_j\, p_j^2}_{T_3} + \underbrace{\alpha^*_j \alpha^*_k (\alpha_j^2 + \alpha_k^2)\, p_j p_k}_{T_4}. \label{eq:decomp}
\end{align}

\textbf{Case $p_j p_k \leq 0$.} Then LHS $\geq 0$ (each summand is $(\text{pos})\cdot p^2\cdot(\text{pos})$) and RHS $= p_j p_k(2-\alpha_j^2-\alpha_k^2) \leq 0$ (since $2-\alpha_j^2-\alpha_k^2 > 0$ for $\alpha_j,\alpha_k \in (0,1)$). Hence LHS $\geq 0 \geq$ RHS.

\textbf{Case $p_j p_k > 0$.} Terms $T_1, T_2, T_3 \geq 0$ and $T_4 > 0$. We show $T_2 + T_3 + T_4 \geq 0$, which suffices since $T_1 \geq 0$. Since $p_j p_k > 0$, both have the same sign. Write $\alpha_j = \alpha^*_j + p_j$ and $\alpha_k = \alpha^*_k + p_k$:
\begin{align*}
T_2 + T_3 + T_4 &= (\alpha^*_j)^2(\alpha^*_k + p_k)\alpha^*_k\,p_k^2 + (\alpha^*_k)^2(\alpha^*_j + p_j)\alpha^*_j\,p_j^2 \\
&\quad + \alpha^*_j\alpha^*_k\bigl((\alpha^*_j+p_j)^2 + (\alpha^*_k+p_k)^2\bigr)p_jp_k.
\end{align*}
Since $\alpha_j, \alpha_k \in (0,1)$ we have $\alpha_j = \alpha^*_j + p_j > 0$ and $\alpha_k = \alpha^*_k + p_k > 0$, so all factors involving $\alpha_j, \alpha_k$ are positive. With $p_jp_k > 0$, every term in $T_2 + T_3 + T_4$ is non-negative (using $\alpha^*_j + p_j = \alpha_j > 0$ for $T_2$'s factor $\alpha_k = \alpha^*_k + p_k > 0$, and similarly for $T_3$). Hence $T_2+T_3+T_4 > 0$ and the decomposition~\eqref{eq:decomp} is positive.

The full expansion is verified in Lean as \texttt{pair\_bound\_clear\_denom} (0~sorry).
\end{proof}
...
This avoids any appeal to uniform continuity of $V'$. The Lean formalization uses \texttt{coercive\_drop\_from\_\allowbreak persistence} in \texttt{ContinuumBarbalat.lean}, which takes: non-negativity, antitonicity, body persistence, and tail vanishing.
\end{proof}

\begin{remark}
In the $n$-pole case (Section~\ref{sec:npole}), the stronger exponential rate $V(t+1) \leq e^{-\mu}V(t)$ with $\mu = K c_{\min}\delta\delta^*$ holds via Gronwall comparison (\texttt{comparison\_decay\_\allowbreak interval} in \texttt{GronwallBridge.lean}).
\end{remark}
...
% ============================================================
\subsection{Step 8: Removing the basin condition (DCT bootstrap)}\label{sec:bootstrap}
% ============================================================

This step is not used in Theorem~\ref{thm:main} (which assumes the basin condition) but is the key additional ingredient for Theorem~\ref{thm:global}.

\begin{proposition}[Uniform positivity, \texttt{r\_stays\_positive\_supercritical}]\label{prop:rpos}
Under hypotheses (i), (ii), (iv) of Theorem~\ref{thm:main}, if $K > K_c := 2/\int(1/\gamma)\,\mathrm{d}\mu$ and $\int(1/\gamma)\,\mathrm{d}\mu < \infty$, then there exists $r_{\min} > 0$ such that $r(t) \geq r_{\min}$ for all $t \geq 0$.
\end{proposition}
...
\]
Each $G_n$ is dominated by $K/(2\gamma(\omega))$, which is integrable (since $\int(1/\gamma)\,\mathrm{d}\mu < \infty$). Since $\alpha(\omega,0) > 0$ and $\gamma(\omega) > 0$, pointwise $G_n(\omega) \to K/(2\gamma(\omega))$ as $n \to \infty$. The dominated convergence theorem gives $\int G_n\,\mathrm{d}\mu \to \int K/(2\gamma)\,\mathrm{d}\mu = (K/2)\int(1/\gamma)\,\mathrm{d}\mu > 1$ (by $K > K_c$).

Extract $N$ with $\int G_N\,\mathrm{d}\mu > 1$ and $1/(N{+}1) < r(0)$. Set $\varepsilon_0 := 1/(N{+}1)$. Then $\varepsilon_0 < r(0)$, $\varepsilon_0 \leq 1$. Dividing $G_N$ by $(N{+}1)$:
\[
\frac{G_N(\omega)}{N{+}1} = \min\!\bigl(\alpha(\omega,0),\; K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0)\bigr),
\]
so $\int G_N\,\mathrm{d}\mu > 1$ gives $\varepsilon_0 = 1/(N{+}1) < (1/(N{+}1))\int G_N\,\mathrm{d}\mu = \int\min(\alpha(\omega,0),\,K\varepsilon_0/(2\gamma+K\varepsilon_0))\,\mathrm{d}\mu$.

\textbf{Step B (Self-improvement).}
If $r(s) \geq \varepsilon_0$ for all $s \in [0,T]$, then for each $\omega$: either $\alpha(\omega,0) \geq \beta^*(\gamma(\omega),K,\varepsilon_0)$ (the body equilibrium), in which case the ODE barrier gives $\alpha(\omega,T) \geq \beta^* \geq K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0)$; or $\alpha(\omega,0) < \beta^*$, in which case monotonicity below equilibrium gives $\alpha(\omega,T) \geq \alpha(\omega,0)$. In either case, $\alpha(\omega,T) \geq \min(\alpha(\omega,0),\, K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0))$. Integrating: $r(T) = \int\alpha(\omega,T)\,\mathrm{d}\mu > \varepsilon_0$.

\textbf{Step C (Contradiction closure).}
Suppose $r(t_0) < \varepsilon_0$ for some $t_0 > 0$. The set $\{t \geq 0 : r(t) \leq \varepsilon_0\}$ is closed (continuity of $r$) and bounded below; let $T = \inf\{t \geq 0 : r(t) \leq \varepsilon_0\}$. Then $T > 0$ (since $r(0) > \varepsilon_0$), $r(T) = \varepsilon_0$, and $r(s) \geq \varepsilon_0$ for all $s \in [0,T]$. Step~B gives $r(T) > \varepsilon_0$, contradicting $r(T) = \varepsilon_0$.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:global}]
Proposition~\ref{prop:rpos} gives $r(t) \geq r_{\min} > 0$ for all $t \geq 0$. This uniform positivity provides the persistence hypotheses needed in the body-tail/Barbalat argument (Proposition~\ref{prop:drop}): on $\{\gamma \leq M\}$, the ODE barrier gives $\alpha(\omega,t) \geq \delta(M) > 0$ for all $t$, using $r \geq r_{\min}$ in place of the Cauchy--Schwarz bound from the basin condition. The body-tail split then gives $V(t) \to 0$ (Proposition~\ref{prop:vzero}). Finally, Proposition~\ref{prop:rconv} gives $r(t) \to r^*$.

In the Lean formalization, this is assembled as \texttt{hPsi\_floor\_of\_r\_liminf}, which reuses the body persistence $+$ Barbalat infrastructure of \texttt{kuramoto\_V\_zero\_of\_r\_floor} (a shared subroutine, not a circular invocation of Theorem~\ref{thm:main}).
\end{proof}
...
\begin{multline*}
(K,\,\gamma,\,r_0) \;\xrightarrow{\text{Bernoulli}}\; \texttt{LorentzianContinuousSolution} \;\to\; \texttt{LorentzianSolution} \\
\to\; \texttt{KuramotoData} \;\to\; r(t) \to r^*.
\end{multline*}
...
The formalization spans \textbf{255 Lean~4 files} (commit \texttt{58b6194}, Mathlib v4.30.0-rc1). The declaration \texttt{real\_scalar\_complete} depends on no project axioms and no \texttt{sorry}; the imported development contains unrelated \texttt{sorry}s in theorems outside this dependency chain. The formalization uses only Lean~4's standard foundations.

There are three tiers of completeness:

\medskip\noindent\textbf{Tier 1 --- Lorentzian (fully end-to-end).} The theorem \texttt{lorentzian\_ode\_global\_\allowbreak stability\_complete} takes only the physical parameters $(K, \gamma, r_0)$ and constructs the complete proof: solution existence via the explicit Bernoulli formula, all solution properties, and convergence $r \to r^*$. Zero axioms, zero assumed fields.
...
Mon.\ bound (Prop.~\ref{prop:rate}) & \texttt{hV\_rate} & \texttt{ContinuumUniformRate} \\
$V$ antitone (Prop.~\ref{prop:antitone}) & \texttt{lyapunov\_antitone} & \texttt{GeneralGMainTheorem} \\
Exp.\ drop (Prop.~\ref{prop:drop}) & \texttt{supercritical\_coercive\_drops} & \texttt{GeneralGMainTheorem} \\
$V \!\to\! 0$ (Prop.~\ref{prop:vzero}) & \texttt{coercive\_drop\_from\_persistence} & \texttt{ContinuumBarbalat} \\
...
\medskip\noindent To verify: \texttt{cd kuramoto-lean \&\& lake build KuramotoLean.RealScalarComplete} (commit \texttt{58b6194}).
...
For $n$-pole systems (any finite $n$), the theorem \texttt{kuramoto\_solved} gives \emph{global} stability: any $r(0) > 0$ converges. Persistence is automatic in finite dimensions. For the continuum scalar dissipative model~\eqref{eq:oa}, global stability is now also proved: \texttt{r\_stays\_positive\_super\-critical} in \texttt{ContinuumInstability.lean} (0~sorry) shows that for $K > K_c$, the order parameter stays uniformly positive ($r(t) \geq r_{\min} > 0$) via a DCT bootstrap argument. The proof constructs $\varepsilon_0 > 0$ such that
$\int\min(\alpha(\omega,0),\, K\varepsilon_0/(2\gamma(\omega)+K\varepsilon_0))\,\mathrm{d}\mu > \varepsilon_0$,
then uses body persistence to show $r$ can never first cross below $\varepsilon_0$ (IVT + closed crossing set contradiction). This removes the basin condition $V(0) < (r^*)^2$ for the scalar dissipative model.
\end{remark}
...
The pair bound argument of Sections~\ref{sec:proof}--\ref{sec:cauchy} applies directly to the scalar model~\eqref{eq:oa}. We now examine what structural elements carry over to the standard complex OA equation~\eqref{eq:complex-oa}. The rotation term admits an exact cancellation in the $V$ derivative, but the remaining nonlinear coupling terms do not reduce to the scalar pair-bound structure. The complex OA convergence is therefore \emph{conditional} on external nonlinear Landau damping estimates.

\subsection{Dietert energy identity}

\begin{proposition}[\texttt{psi\_deriv\_eq\_K\_eta\_sq}, 0~sorry]\label{prop:psi}
Define $\Psi(t) := -\!\int \log(1-|z(\omega,t)|^2)\,g\,\mathrm{d}\omega$. Then
\[
\frac{\mathrm{d}\Psi}{\mathrm{d}t} = K|\eta(t)|^2 \geq 0.
\]
\end{proposition}
\begin{proof}
By chain rule, $\frac{\mathrm{d}}{\mathrm{d}t}[-\log(1-|z|^2)] = \frac{\mathrm{d}|z|^2/\mathrm{d}t}{1-|z|^2}$. From the complex OA, $\mathrm{d}|z|^2/\mathrm{d}t = K\,\re(\eta z)(1-|z|^2)$ (\texttt{complexOa\_normSq\_\allowbreak deriv}, 0~sorry). Hence the pointwise integrand is $K\,\re(\eta z)$. Integrating: $\dot\Psi = K\!\int\! \re(\eta z)\,g = K\,\re(\eta\bar\eta) = K|\eta|^2$, using $\bar\eta = \int z\,g$ for real-valued~$g$ (\texttt{complex\_eta\_integral\_identity}, 0~sorry).
\end{proof}
...
The rotation cancellation removes the rotation contribution from $V'$, but the remaining nonlinear coupling terms differ structurally from the scalar case: the coupling involves $\eta \in \C$ (not $r \in \R$), and the quadratic terms $\eta^*(z^{*2} - z^2)$ do not factor as cleanly as in the scalar pair bound. The scalar pair-bound technique does \emph{not} extend to the complex OA. Convergence $V(t) \to 0$ is instead obtained from nonlinear Landau damping \cite{dietert2017}: for Sobolev-regular perturbations of the PLS, phase mixing of drifting oscillators gives algebraic decay $|V(t)| = O(t^{-s})$ (stated as axiom \texttt{landau\_damping\_kuramoto}). This is a \emph{conditional} result relying on external analytic input, not a self-contained machine-checked proof.
...
\subsection{Formalization tiers}
...
\medskip
\noindent\textbf{Theorem A$'$: Real scalar global} (Theorem~\ref{thm:global}). Fully formalized, 0~sorry, 0~axioms. \textbf{Global} stability for the scalar dissipative model: for $K > K_c$ and $\alpha(\omega,0) \in (0,1)$, the order parameter converges $r(t) \to r^*$ \emph{without} assuming $V(0) < (r^*)^2$. The DCT bootstrap proves $r(t) \geq r_{\min} > 0$ uniformly, entering the basin machinery of Theorem~A.
...
\medskip
\noindent\textbf{Theorem C: $n$-pole} (\texttt{kuramoto\_solved}). Fully formalized, 0~sorry, 0~axioms. Finite-dimensional, exponential decay rate $\mu = Kc_{\min}\delta\delta^*$. Global stability for any~$r(0) > 0$.
...
\textbf{Name} & \textbf{Model} & \textbf{Hypotheses} & \textbf{Conclusion} & \textbf{Sorry} & \textbf{Ax.} \\
\hline
\texttt{real\_scalar\_complete} & Scalar diss. & $\int\gamma\,g < \infty$, $V(0) < (r^*)^2$ & $r(t) \to r^*$ & 0 & 0 \\
\texttt{real\_scalar\_global} (Thm.~\ref{thm:global}) & Scalar diss. & $\int\gamma + \int 1/\gamma < \infty$, $K > K_c$ & $r(t) \to r^*$ & 0 & 0 \\
\texttt{lorentzian\_ode\_global\_stability\_complete} & Lorentzian & $K > 2\gamma$, $r_0 \in (0,1)$ & $r(t) \to r^*$ & 0 & 0 \\
...
\subsection{Significance of the scalar dissipative result}

The critical coupling $K_c = 2/\int(1/\gamma)\,\mathrm{d}\mu$, the equilibrium profile $\alpha^*(\omega)$, and the order parameter $r^*$ are \emph{identical} between the scalar dissipative model~\eqref{eq:oa} and the standard Kuramoto--OA system~\eqref{eq:complex-oa} (restricted to the positive real symmetric subspace). The global stability theorem (Theorem~\ref{thm:global}) therefore proves that the same positive real steady state is a global attractor for a scalar dissipative flow sharing the Kuramoto phase-transition threshold, the Kuramoto fixed point, and the Kuramoto self-consistency structure. The pair bound is a property of the self-consistency algebra (the equilibrium relation~\eqref{eq:equil} and the integral constraint~\eqref{eq:sc}), not the specific flow generating the transients. This makes the technique potentially transferable to other mean-field models with the same equilibrium structure.
...
\begin{enumerate}
\item \textbf{Novel proof technique for the scalar dissipative model.} The pair bound (Proposition~\ref{prop:pair})---a pointwise algebraic inequality lifted to the continuum double integral via Fubini---has no precedent in the Kuramoto literature. Prior approaches to Kuramoto-type stability used Fourier analysis \cite{dietert2016}, Volterra equations \cite{dietert-fernandez2018}, or spectral theory \cite{chiba2015}. The pair bound applies to the scalar dissipative model~\eqref{eq:oa}; it does not directly apply to the standard complex OA~\eqref{eq:complex-oa}.

\item \textbf{Machine-checked formalization.} To the best of our knowledge, this is the first Lean formalization of a stability theorem for a Kuramoto-type model. (We have not found competing formalizations in Coq, Isabelle, HOL Light, or Agda, but do not claim to have performed an exhaustive search.)

\item \textbf{Rotation cancellation (structural observation).} The rotation term $-i\omega$ in the complex OA cancels in the $V$~derivative after equilibrium subtraction (Proposition~\ref{prop:rotation}). This is a structural observation relevant to the complex OA Lyapunov analysis; it does not by itself yield convergence (which requires external Landau damping input).

\item \textbf{Finite first moment coverage.} Many explicit OA reductions and fully closed finite-dimensional analyses rely on Lorentzian or rational distributions. The finite first moment condition $\int \gamma\,\mathrm{d}\mu < \infty$ (with $\gamma(\omega) \lesssim 1+|\omega|$) covers Student-$t$ with $\nu > 1$ (including the infinite-variance case $1 < \nu \leq 2$), Gaussian, compact support, and mixtures.
\end{enumerate}

\subsection{Limitations}

\begin{enumerate}
\item \textbf{Basin of attraction (resolved for the scalar dissipative model).} The basin stability result (Theorem~\ref{thm:main}) requires $V(0) < (r^*)^2$. This condition is discharged by Theorem~\ref{thm:global} for the scalar dissipative model~\eqref{eq:oa}: the DCT bootstrap proves $r(t) \geq r_{\min} > 0$ for $K > K_c$, giving global stability without a basin condition. The restriction to the scalar dissipative model remains (not the standard complex OA~\eqref{eq:complex-oa}).
...
\begin{enumerate}
\item \sout{Remove the $V(0) < (r^*)^2$ basin condition.} \textbf{Resolved} for the scalar dissipative model: \texttt{r\_stays\_positive\_supercritical} (0~sorry) proves $r(t) \geq r_{\min} > 0$ for $K > K_c$ via DCT bootstrap. Remains open for the standard complex OA equation~\eqref{eq:complex-oa} without the Landau damping axiom.
...
\textbf{Final theorems} & \texttt{real\_scalar\_complete} (basin), \\
& \texttt{r\_stays\_positive\_supercritical} (global $r > 0$), \\
& \texttt{hPsi\_floor\_of\_r\_liminf} (bridge), \\
& \texttt{lorentzian\_ode\_global\_stability\_complete} \\
\textbf{Axiom check} & \texttt{\#print axioms real\_scalar\_complete} \\
& \texttt{\#print axioms r\_stays\_positive\_supercritical} \\
& \texttt{\#print axioms hPsi\_floor\_of\_r\_liminf} \\
& \texttt{\#print axioms lorentzian\_ode\_global\_stability\_complete} \\
& All output only \texttt{propext}, \texttt{Quot.sound}, \texttt{Classical.choice} \\
\textbf{Sorry check} & \texttt{grep -rn "\^{}\textbackslash s*sorry" KuramotoLean/} \\
& returns 0 matches in the stability chain files \\
\textbf{Unrelated sorry} & \texttt{NeuralMeanField.lean} (2), \texttt{MeanFieldLimit.lean} (4) --- \\
& outside the dependency closure of any final theorem above
\end{tabular}

\medskip\noindent The final theorems transitively depend on no project axioms and no \texttt{sorry}. The unrelated sorry's in imported files are in theorems that do not appear in the dependency graph of any theorem listed above, verifiable via \texttt{\#print axioms}.
