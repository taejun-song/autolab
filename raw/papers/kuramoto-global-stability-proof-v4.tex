...
The rotation cancellation removes the rotation contribution from $V'$, but the remaining nonlinear coupling terms differ structurally from the scalar case: the coupling involves $\eta \in \C$ (not $r \in \R$), and the quadratic terms $\eta^*(z^{*2} - z^2)$ do not factor as cleanly as in the scalar pair bound. Convergence $V(t) \to 0$ for the complex OA is not proved here; it would require an external input such as nonlinear Landau damping~\cite{dietert2017} (see Section~\ref{sec:bridge} and Remark~\ref{rem:bridge-hypotheses} for a discussion of what this entails).
...
\begin{lemma}[Formal order-parameter bridge, \texttt{full\_kuramoto\_pde\_stability\_via\_axiom}, 0 sorry]\label{lem:bridge}
...
\medskip
\noindent\textbf{Lemma D: Conditional bridge} (\texttt{kuramoto\_stability\_universal}). Formal bridge lemma (reverse triangle inequality), 0~sorry, 2~assumptions. The two assumptions---OA order-parameter convergence and PDE-to-OA closeness---are encoded as \texttt{opaque Prop} types in Lean. Discharging them would require matching the hypotheses and conclusions of~\cite{dietert2017} (Landau damping) and~\cite{dietert-fernandez2018} (OA attractivity) to the present notation; this matching is not carried out here (see Remark~\ref{rem:bridge-hypotheses}).
...
\texttt{kuramoto\_stability\_universal} & Conditional bridge & 2 unmatched assumptions (Remark~\ref{rem:bridge-hypotheses}) & $|R_{\mathrm{full}}| \to r_*$ & 0 & 2 \\
\end{tabular}
\end{center}
...
\item \textbf{Unmatched external assumptions.} Lemma~D is only the reverse triangle inequality. Its two assumptions---OA convergence and PDE-to-OA closeness---would need to be derived from~\cite{dietert2017} (Landau damping) and~\cite{dietert-fernandez2018,engelbrecht2020} (OA attractivity) respectively, but the matching (phase conventions, rotating frames, trace estimates, smallness/regularity verification) is not carried out here. The assumptions are encoded as \texttt{opaque Prop} types in Lean that cannot be trivially discharged. See Remark~\ref{rem:bridge-hypotheses} for details.
