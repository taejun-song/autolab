...
The formalization spans \textbf{258 Lean~4 files} (commit \texttt{7f95a97}, Mathlib v4.30.0-rc1). It gives a no-sorry Lean formalization of the pair-bound Lyapunov proof for the scalar dissipative Kuramoto-type mean-field equation. The extension to the standard complex OA equation and the full Kuramoto PDE is conditional on external convergence and attractivity inputs that are not proved in this paper (see Remark~\ref{rem:bridge-hypotheses}). The declaration \texttt{real\_scalar\_complete} depends on no project axioms and no \texttt{sorry}; the imported development contains unrelated \texttt{sorry}s in theorems outside this dependency chain.
...
\begin{remark}[Hypotheses of Theorem~\ref{thm:full}]\label{rem:bridge-hypotheses}
Theorem~\ref{thm:full} is a conditional bridge. Its conclusion follows from the reverse triangle inequality once two independent assumptions are supplied: convergence of the OA order parameter to the target amplitude, and asymptotic closeness of the full PDE order parameter to the OA order parameter. These assumptions are not proved in this paper.

To discuss attraction to the OA manifold for the full Kuramoto PDE, the natural objects are Fourier-side OA defects. For the Fourier modes $\hat f_\ell(\tau) = \int_{\mathbb{T} \times \mathbb{R}} e^{-i(\ell\theta + \tau\omega)}\,f(\mathrm{d}\theta,\mathrm{d}\omega)$, the OA manifold is characterized by the convolution identities $\hat f_\ell = h^{*\ell} * \hat g$ for $\ell \geq 0$. The OA defect variables
\[
w_{n,m} := \hat f_{n+m} * \hat g - \hat f_n * \hat f_m
\]
vanish on the OA manifold. Their size is measured in a weighted Fourier--Sobolev norm of the form
\[
\|w\|_{H^1_{e^{a\tau}}}^2 = \sum_{n,m \geq 1} \int_\R \frac{e^{2a\tau}}{nm}\bigl(|w_{n,m}(\tau)|^2 + |\partial_\tau w_{n,m}(\tau)|^2\bigr)\,\mathrm{d}\tau.
\]
Dietert--Fernandez OA attractivity~\cite{dietert-fernandez2018} gives exponential decay $\|w(t)\|_{H^1_{e^{a\tau}}} \leq \|w(0)\|_{H^1_{e^{a\tau}}}\,e^{-at}$ in this topology. This is structurally different from a pointwise bound on an OA parameter $\alpha(\omega,t)$ in a strip: the defect norm is a global Fourier--Sobolev quantity, not a pointwise analytic continuation bound.

Dietert's nonlinear Landau damping~\cite{dietert2017} proves local orbital stability of linearly stable partially locked states for the full PDE after removing the neutral rotation mode, requiring linear stability, Sobolev smallness, and regularity of~$g$. Neither external result is stated in the variables or topology of the present paper, and matching them to the present notation remains open. The full chain is \texttt{kuramoto\_stability\_universal} (0~sorry, 2~axioms).
\end{remark}
...
\medskip
\noindent\textbf{Theorem C: $n$-pole} (\texttt{kuramoto\_solved}). Fully formalized, 0~sorry, 0~axioms. Finite-dimensional, exponential decay rate $\mu = K\delta\delta^*$. Global stability for any $r(0) > 0$.

\medskip
\noindent\textbf{Theorem D: Complex OA + full PDE} (\texttt{kuramoto\_stability\_universal}). Conditional bridge, 0~sorry, 2~axioms. The complex OA convergence relies on nonlinear Landau damping \cite{dietert2017} (axiom); the PDE-to-OA bridge relies on OA manifold attractivity \cite{dietert-fernandez2018} (axiom). Both axioms have opaque (non-vacuous) conditions encoded as \texttt{opaque Prop} types. The external results are cited but their hypotheses (linear stability, Sobolev smallness, weighted Fourier--Sobolev defect norms) have not been matched to the present notation.
...
\texttt{kuramoto\_stability\_universal} & Complex OA + PDE & Analytic $g$, $K > K_c$ + 2 cited axioms & $\|\eta\| \to r^*$ & 0 & 2 \\
\end{tabular}
\end{center}
