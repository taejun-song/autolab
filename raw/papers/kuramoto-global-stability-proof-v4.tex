...
\item \textbf{Unmatched external assumptions.} Lemma~D is only the reverse triangle inequality. Its two mathematical assumptions---OA convergence and PDE-to-OA closeness---would need to be derived from~\cite{dietert2017} (Landau damping) and~\cite{dietert-fernandez2018,engelbrecht2020} (OA attractivity) respectively, but the matching (phase conventions, rotating frames, trace estimates, smallness/regularity verification) is not carried out here. In Lean, OA convergence is a regular hypothesis; PDE-to-OA closeness is encoded via one \texttt{opaque} type and one \texttt{axiom}. See Remark~\ref{rem:bridge-hypotheses} for details.
