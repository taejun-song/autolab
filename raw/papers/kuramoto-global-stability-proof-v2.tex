...
Theorem~\ref{thm:full} is a conditional bridge. Its conclusion follows from the reverse triangle inequality once two independent assumptions are supplied: convergence of the OA order parameter to the target amplitude, and asymptotic closeness of the full PDE order parameter to the OA order parameter. These assumptions are not proved in this paper. Dietert's nonlinear Landau damping~\cite{dietert2017} proves local orbital stability of linearly stable partially locked states for the full PDE after removing the neutral rotation mode; Dietert--Fernandez OA attractivity~\cite{dietert-fernandez2018} is formulated in a weighted Fourier--Sobolev topology for OA defects. These external results are cited as opaque axioms; their hypotheses (linear stability, Sobolev smallness, regularity of~$g$, weighted defect norms) have not been matched to the present notation. The full chain is \texttt{kuramoto\_stability\_universal} (0~sorry, 2~axioms).
\end{remark}
