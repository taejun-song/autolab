...
\begin{abstract}
We present a machine-checked proof of stability of the partially locked state (PLS) for the Kuramoto model on the Ott--Antonsen (OA) manifold. The formalization operates at two levels. \textbf{Level~1} (scalar dissipative model): for a mean-field equation $\dot\alpha = -\gamma(\omega)\alpha + (K/2)r(1-\alpha^2)$ with finite first moment and initial data in an explicit basin ($V(0) < (r^*)^2$), the order parameter converges $r(t) \to r^*$ via a novel pair bound and Barbalat argument. \textbf{Level~2} (standard complex OA): for the standard Ott--Antonsen equation $\dot z = -i\omega z + (K/2)(\bar\eta - \eta z^2)$, we prove the Dietert energy identity $\mathrm{d}\Psi/\mathrm{d}t = K|\eta|^2 \geq 0$, symmetry preservation ($z(\omega) = \overline{z(-\omega)}$ invariant), and---crucially---that the rotation term $-i\omega$ \emph{cancels} in the $V$~derivative after equilibrium subtraction, making the pair bound applicable to the standard Kuramoto reduction.

The Lean~4 formalization spans \textbf{235+ files} with \textbf{0~\texttt{sorry}} and only Lean's standard axioms plus one cited axiom (OA manifold attractivity \cite{dietert-fernandez2018}). The complex OA convergence is proved unconditionally via a body-tail decomposition: rotation cancellation enables the body pair bound, body persistence gives exponential decay, and tail vanishing completes the argument (\texttt{complex\_oa\_V\_zero\_unconditional}, 0~sorry). Coverage: Lorentzian (global, scalar ODE), $n$-pole (global, any finite~$n$), complex OA with symmetric~$g$ (full convergence via body-tail split), and all finite-first-moment distributions (local stability via pair bound). Combined with OA manifold exponential attractivity, this yields stability for the full Kuramoto--Sakaguchi PDE for analytic~$g$. The pair bound---a pointwise SOS inequality lifted to the continuum via Fubini---and the rotation cancellation identity are novel proof techniques with no precedent in the Kuramoto literature. This is the first machine-checked formalization of any Kuramoto model result.
\end{abstract}
...
After the rotation vanishes, $V'$ depends only on the $K$-coupling terms. On each body region $\{|\omega| \leq M\}$, the bounded rotation allows the pair bound to apply, giving $V_{\mathrm{body}}'(t) \leq 0$. Combined with body persistence and tail vanishing ($|z-z^*|^2 \leq 4$ with $\int_{|\omega|>M} g \to 0$), this yields $V(t) \to 0$ via the body-tail $\varepsilon/2$ argument (\texttt{complex\_oa\_V\_zero\_unconditional}, 0~sorry).
...
\end{proposition}
\begin{proof}
Define $w(\omega,t) := \overline{z(-\omega,t)}$. Conjugating the ODE and using $g(-\omega) = g(\omega)$ gives $\dot w(\omega,t) = -i\omega w + (K/2)(\bar\eta - \eta w^2)$~--- the same ODE as~$z(\omega)$ (\texttt{complexOaRHS\_conj\_neg}, 0~sorry). Since $w(\omega,0) = z(\omega,0)$, ODE uniqueness gives $w = z$.
\end{proof}

\subsection{Full Kuramoto PDE stability}

\begin{theorem}[\texttt{full\_kuramoto\_pde\_stability}, 0 sorry + 1 axiom]\label{thm:full}
...
The hypothesis $\|\eta_{\mathrm{full}} - \eta_{\mathrm{OA}}\| \to 0$ follows from OA manifold exponential attractivity \cite{dietert-fernandez2018} (stated as axiom \texttt{oa\_manifold\_attractivity}). The hypothesis $\|\eta_{\mathrm{OA}}\| \to r^*$ is now fully machine-checked: rotation cancellation (Proposition~\ref{prop:rotation}) enables the body-restricted pair bound, body persistence and Barbalat give $V_{\mathrm{body}} \to 0$, and tail vanishing gives $V_{\mathrm{tail}} \to 0$. The full chain is \texttt{complex\_oa\_V\_zero\_unconditional} (0~sorry).
\end{remark}
...
\item \textbf{Perturbative coercivity condition.} The complex OA convergence (\texttt{kuramoto\_stability\_perturbative}) assumes a quantitative condition: the real pair bound coercivity exceeds the imaginary error ($c_{\mathrm{coercive}} > C_{\mathrm{error}}$). This is checkable for any specific $(K, g)$ but not proved universally for all $K > K_c$. It holds at strong coupling ($K \gg K_c$) but may fail near onset.

\item \textbf{OA manifold attractivity.} Theorem~\ref{thm:full} uses OA manifold exponential attractivity \cite{dietert-fernandez2018} as a cited axiom, not a formalized proof.
...
\item Prove the perturbative coercivity condition $c_{\mathrm{coercive}} > C_{\mathrm{error}}$ universally for all $K > K_c$ (currently a checkable hypothesis per distribution).
