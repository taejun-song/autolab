...
\item \textbf{OA manifold attractivity.} Theorem~\ref{thm:full} uses OA manifold exponential attractivity \cite{dietert-fernandez2018} as a cited axiom, not a formalized proof. Full formalization of Dietert--Fernandez's spectral theory remains future work.

\item \textbf{Scalar model scope.} The scalar dissipative model~\eqref{eq:oa} is a separate dynamical system from the standard complex OA~\eqref{eq:complex-oa}. The identification $\gamma(\omega) = |\omega|$ is specific to the Lorentzian pole-reduction mechanism and does not constitute a general reduction from the complex OA.
\end{enumerate}

\subsection{Open problems}

\begin{enumerate}
\item Remove the $V(0) < (r^*)^2$ condition via $\Psi$~monotonicity or passage to the limit from $n$-pole.
