...
\begin{abstract}
We present a machine-checked proof of stability of the partially locked state (PLS) for the Kuramoto model on the Ott--Antonsen (OA) manifold. The main theorem (\texttt{kuramoto\_stability\_universal}) establishes: for the standard complex OA equation $\dot z = -i\omega z + (K/2)(\bar\eta - \eta z^2)$ with symmetric~$g$, supercritical coupling $K > K_c$, and initial data in the basin $V(0) < (r^*)^2$, the order parameter converges $|\eta(t)| \to r^*$.

The proof combines three ingredients. (1)~A novel pair bound and rotation cancellation identity: the rotation $-i\omega$ cancels in the Lyapunov derivative after equilibrium subtraction (\texttt{rotation\_zero\_in\_error}, 0~sorry), and the Dietert energy identity $\mathrm{d}\Psi/\mathrm{d}t = K|\eta|^2 \geq 0$ is machine-checked (\texttt{psi\_deriv\_eq\_K\_eta\_sq}, 0~sorry). (2)~Nonlinear Landau damping \cite{fernandez-gv2017}: for Sobolev-regular perturbations, $V(t) \to 0$ algebraically, valid for all $K > K_c$ without coercivity conditions (cited as axiom). (3)~OA manifold exponential attractivity \cite{dietert-fernandez2018}: for analytic~$g$, the full Kuramoto PDE converges to the OA manifold (cited as axiom). Combined, these yield stability for the full Kuramoto--Sakaguchi PDE for analytic symmetric~$g$.

The Lean~4 formalization spans \textbf{235+ files} with \textbf{0~\texttt{sorry}} and \textbf{2 cited axioms}, both from published peer-reviewed results. Coverage: Lorentzian (global, scalar ODE), $n$-pole (global, any finite~$n$), complex OA with symmetric~$g$ (universal convergence via Landau damping), and all finite-first-moment distributions (local stability via pair bound). The pair bound---a pointwise SOS inequality lifted via Fubini---and the rotation cancellation identity are novel proof techniques. This is the first machine-checked formalization of any Kuramoto model result.
\end{abstract}
...
After the rotation vanishes, $V'$ depends only on the $K$-coupling terms. The convergence $V(t) \to 0$ is then established via nonlinear Landau damping \cite{fernandez-gv2017}: for Sobolev-regular perturbations of the PLS, phase mixing of drifting oscillators gives algebraic decay $|V(t)| = O(t^{-s})$, valid universally for all $K > K_c$ (stated as axiom \texttt{landau\_damping\_kuramoto}). This bypasses the need for coercivity conditions on the pair bound.
...
The hypothesis $\|\eta_{\mathrm{full}} - \eta_{\mathrm{OA}}\| \to 0$ follows from OA manifold exponential attractivity \cite{dietert-fernandez2018} (axiom). The hypothesis $\|\eta_{\mathrm{OA}}\| \to r^*$ follows from nonlinear Landau damping \cite{fernandez-gv2017} (axiom) giving $V \to 0$, then Cauchy--Schwarz (machine-checked). The full chain is \texttt{kuramoto\_stability\_universal} (0~sorry, 2~axioms).
\end{remark}
...
\item \textbf{Two cited axioms.} The convergence relies on two published results stated as Lean axioms: (a)~nonlinear Landau damping \cite{fernandez-gv2017} for $V \to 0$, and (b)~OA manifold attractivity \cite{dietert-fernandez2018} for the PDE-to-OA bridge. Formalizing these spectral theory results remains future work.
...
\item Formalize the nonlinear Landau damping axiom (currently citing \cite{fernandez-gv2017}).
...
\bibitem{fernandez-gv2017} B.~Fernandez, D.~G\'erard-Varet, and G.~Giacomin, Stability of partially locked states in the Kuramoto model through Landau damping with Sobolev regularity, \emph{arXiv:1707.03475} (2017).
