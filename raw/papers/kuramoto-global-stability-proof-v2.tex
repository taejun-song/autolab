...
After the rotation vanishes, $V'$ depends only on the $K$-coupling terms. Whether the pair bound (Proposition~\ref{prop:pair}) extends to complex~$z \in \D$ remains an open question (see Open Problem~3). The antitonicity $V'(t) \leq 0$ for the complex OA is \textbf{not yet machine-checked}.
...
The hypothesis $\|\eta_{\mathrm{full}} - \eta_{\mathrm{OA}}\| \to 0$ follows from OA manifold exponential attractivity \cite{dietert-fernandez2018} (stated as axiom \texttt{oa\_manifold\_attractivity}). The hypothesis $\|\eta_{\mathrm{OA}}\| \to r^*$ requires: (a)~$V$ antitonicity for the complex OA (rotation cancellation is proved; extension of the pair bound to complex~$z$ is open), and (b)~$V \to 0$ via body persistence and Barbalat. For the scalar model~\eqref{eq:oa}, both (a) and (b) are machine-checked (0~sorry).
\end{remark}
...
\begin{enumerate}
\item Remove the $V(0) < (r^*)^2$ condition for the complex OA via $\Psi$~monotonicity or passage to the limit from $n$-pole.
\item Extend to distributions without finite first moment (Student-$t$ with $\nu \leq 1$) beyond the Lorentzian special case.
\item Formalize the complex pair bound: prove $V'(t) \leq 0$ for~\eqref{eq:complex-oa} by combining rotation cancellation (Proposition~\ref{prop:rotation}) with the SOS inequality (Proposition~\ref{prop:pair}) for complex~$z$.
\end{enumerate}
