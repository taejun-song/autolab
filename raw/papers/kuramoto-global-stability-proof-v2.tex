...
After the rotation vanishes, $V'$ depends only on the $K$-coupling terms, which have identical algebraic structure to the scalar case. The pair bound (Proposition~\ref{prop:pair}) extends directly, giving $V'(t) \leq 0$.
...
For the full Kuramoto--Sakaguchi PDE with analytic symmetric~$g$, $K > K_c$, and initial data near the OA manifold: $\|\eta_{\mathrm{full}}(t)\| \to r^*$.
\end{theorem}
\begin{proof}
By OA manifold exponential attractivity \cite{dietert-fernandez2018} (stated as axiom \texttt{oa\_manifold\_attractivity}): $\|\eta_{\mathrm{full}} - \eta_{\mathrm{OA}}\| \to 0$. On the OA manifold: $|\eta_{\mathrm{OA}}| \to r^*$ (from the pair bound + Barbalat). Triangle inequality gives convergence (\texttt{squeeze\_zero\_norm + abs\_norm\_sub\_norm\_le}).
\end{proof}
