...
\]
satisfies: (i)~$z^*$ solves $\dot z = 0$, (ii)~$|z^*| = 1$, (iii)~$\re(z^*) > 0$, and (iv)~the linearized eigenvalue $\lambda = -i\omega - Kr\,z^*$ has $\re(\lambda) = -\sqrt{K^2r^2 - \omega^2} < 0$ (exponential stability).
\end{proposition}
\begin{proof}
Direct substitution: $(Kr/2)(z^{*2} - 1) + i\omega z^* = 0$ reduces to the identity $D^2 + \omega^2 = K^2r^2$ where $D = \sqrt{K^2r^2-\omega^2}$. Parts (ii)--(iv) follow from the explicit formula (\texttt{ComplexOALockedEquil.lean}, 168 lines, 0~sorry).
\end{proof}
