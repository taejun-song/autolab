...
We derive $V'(t) \leq 0$ from the pair bound. Convergence $V(t) \to 0$ is obtained later by body-tail splitting and Barbalat (Proposition~\ref{prop:barbalat}).
...
\textbf{Step B (Body convergence).} Fix $M = M_0$. On $\{\gamma \leq M\}$, body persistence (Proposition~\ref{prop:persist}) gives $\alpha(\omega,t) \geq \delta(M) > 0$. The body-restricted pair coercivity then gives a positive lower bound:
\[
-V'(t) \geq K \cdot \delta(M) \cdot \delta^*(M) \cdot \mu(\gamma \leq M) \cdot V_{\mathrm{body},M}(t),
\]
where $\delta^*(M) = \min_{\gamma \leq M} \alpha^*(\omega) > 0$. Since $V$ is antitone and $V' \leq -c \cdot V_{\mathrm{body},M}$ with $c > 0$: the time-integral $\int_0^\infty V_{\mathrm{body},M}(t)\,\mathrm{d}t$ converges (bounded by $V(0)/c$). Since $V_{\mathrm{body},M}$ is also non-negative and antitone (as a partial integral of an antitone integrand), the integrable antitone function must converge to $0$. (This is a monotone version of Barbalat that avoids uniform continuity of $V'$: antitone $+$ integrable $\Rightarrow$ limit is $0$.)
